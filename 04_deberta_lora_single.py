...
model = DebertaV2ForSequenceClassification.from_pretrained(
    ...
    num_labels=2,
    torch_dtype=torch.float32  # Adding this beacause of  error
)
#LoRA config — Same hyperparameters as Roberta
lora_config = LoraConfig(
    task_type=TaskType.SEQ_CLS,
    r=16,
    lora_alpha=32,
    lora_dropout=0.1,
    target_modules=["query_proj", "value_proj"],#Different name for DeBERTA
    bias="none"
)

model = get_peft_model(model, lora_config)
model = model.to(torch.float32)  # Adding this beacause of  error
...
class SaveCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        path  = f"{DIRS['checkpoints2']}/deberta_lora_single/epoch_{epoch}"
        trainer.model.save_pretrained(path)
        print(f"✓ Epoch {epoch} saved to Drive.")


training_args = TrainingArguments(
    output_dir=f"{DIRS['checkpoints2']}/deberta_lora_single",
    num_train_epochs=3,
    per_device_train_batch_size=8, # Reduced from 16 to 8
    per_device_eval_batch_size=16, # Reduced  from 32 to 15
    learning_rate=2e-5,            # Reduced from 2e-4 to 1e-4  and 1e-4 to 2e-5
    warmup_steps=500,              #Changed 200 to 500
    eval_strategy="epoch",
    save_strategy="epoch",
    load_best_model_at_end=True,
    metric_for_best_model="f1",
    fp16=False, #"DeBERTa-v3 was trained with fp32 due to gradient scaling incompatibilities with fp16, while RoBERTa used fp16. This difference is noted but does not affect the validity of the comparison, as both models were evaluated under their optimal stable training configurations."
    bf16=False,
    seed=SEED,
    report_to="none"
)
...
# Sonuçları kaydet — eğitim bittikten sonra doldir
results = {
    "model": "DeBERTa-v3 + LoRA (Single)",
    "accuracy": 0.9534,
    "f1_score": 0.9536,
    "train_time_minutes": round(train_time/60, 1),
    "train_time_note": "The Colab T4 fp32 is indicative.",
    "trainable_parameters": 591362,
    "total_parameters": 185015044,
    "trainable_percent": 0.3196,
    "epochs": 3,
    "learning_rate": 2e-5,
    "lora_r": 16,
    "lora_alpha": 32,
    "fp16": False,
    "fp16_note": "DeBERTa-v3 fp16 is experiencing gradient underflow"
}

save_results(results, "baseline_deberta_lora_single.json")

model.save_pretrained(f"{DIRS['checkpoints2']}/deberta_lora_single/final")
tokenizer.save_pretrained(f"{DIRS['checkpoints2']}/deberta_lora_single/final")
print("✓ Model saved to Drive.")

NOTEBOOK_NAME = "04_deberta_lora_single"
...
push_to_github("De lora single training completed")
